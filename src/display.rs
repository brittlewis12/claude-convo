...
pub fn format_event_summary(event: &DisplayEvent) -> String {
    let mut summary = String::new();
    
    // Role indicator
    match event.role.as_str() {
        "user" => summary.push_str(&"[USER]".bright_cyan().to_string()),
        "assistant" => summary.push_str(&"[ASSISTANT]".bright_green().to_string()),
        role if role.starts_with("system:") => summary.push_str(&"[SYSTEM]".bright_yellow().to_string()),
        _ => summary.push_str(&format!("[{}]", event.role.to_uppercase()).to_string()),
    }
    
    // Add content preview for user messages
    if event.role == "user" && !event.content.is_empty() {
        let preview = event.content.chars().take(80).collect::<String>();
        summary.push_str(&format!(" {}", preview.white()));
    }
    
    // Add indicators for assistant features
    if event.role == "assistant" {
        if event.thinking.is_some() {
            summary.push_str(&" [THINKING]".bright_magenta().to_string());
        }
        if event.tool_info.is_some() {
            summary.push_str(&" [TOOL]".bright_blue().to_string());
        }
        
        if let Some(usage) = &event.usage {
            summary.push_str(&format!(
                " {}",
                format!("({}→{} tokens)", usage.input_tokens, usage.output_tokens).dimmed()
            ));
        }
    }
...
pub fn print_session_header(session_id: &str, events: &[DisplayEvent]) {
    ...
    println!("│ {} │", format!("Duration: {}m {}s", total_minutes, seconds).white());
    println!("│ {} │", format!("Messages: {}", events.len()).white());
    println!("│ {} │", format!("Tokens: {} in → {} out", total_input, total_output).white());
